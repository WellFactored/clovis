...
case class RSAKeyPair(privateKey: RSAPrivateKey, publicKey: RSAPublicKey) {
  val encodedPrivateKey: String = RSAKeyCodec.encodePrivateKey(privateKey)
  val encodedPublicKey:  String = RSAKeyCodec.encodePublicKey(publicKey)
}

object RSAKeyPair {
  import RSAKeyCodec.{decodePrivateKey, decodePublicKey}
